...
Bumpa Loyalty app is a gamification solution solution to reward customers with great patronage.
...
### 2. Installation
NB: The docker setup is in progress and is intended to make the installation a breeze.
Before installing the ensure you can execute basic `sudo` command. You will only need one `chmod +x install.sh`. 

- If you are on windows, use `wsl` or `git batch`
- Ensure Docker & docker compose are installed and active
- Run `chmod +x install.sh` to give permission to the bash scripts located at `./install.sh`. This script will setup the container, install dependencies and services for both apps and finally run the apps.
